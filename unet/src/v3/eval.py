...
def run_tsne_variants(model: SmallUNetSSL, loader: DataLoader, device: torch.device, out_prefix: str, max_items: int = 1000, label="label"):
    ...
    for mode in modes:
        ...
        for lbl in uniq:
            ...
            if label == "label_1":
                name_id = mindset_label_map_idx_1.get(key, key)
                name = mindset_idx_map_label_1.get(name_id, name_id)
                color = mindset_colors_1.get(name, "#888888")
            elif label == "label_2":
                name_id = mindset_label_map_idx_2.get(key, key)
                name = mindset_idx_map_label_2.get(name_id, name_id)
